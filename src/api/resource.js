...
export const fetchResourceTags = async () => {
  try {
    ...
    return {
      success: false,
      data: [],
      error: error.response?.data?.message || error.message || "Failed to fetch resource tags",
      status: error.response?.status || 500
    };
  }
};
...
export const fetchResourcesByTag = async (tag) => {
  try {
    ...
    return {
      success: false,
      data: [],
      error: error.response?.data?.message || error.message || "Failed to fetch resources by tag",
      status: error.response?.status || 500
    };
  }
};
